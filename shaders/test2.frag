...
float scene(vec3 p)
{
    ...
    pp=p;
    pp=erot(pp,vec3(0,1,0),iTime+pp.y*.01);
    moda(pp.xz,5);
    pp.x-=23;
    pp=erot(pp,vec3(0,1,0),iTime+pp.y*.01);
    float bars=bar(pp.xz,vec2(2.5,.5));
    pp=p;
    pp.y=mod(pp.y,80)-40;
    float top=cylinder(pp,30,1);
...
void main()
{
    ...
    if(d<threshold)
    {
        ...
        o_fragment=sqrt((vec4(color,1)*light)*.5);
    }
    else
    {
        uv.y=uv.y*.5+.5;
