...
float logo(vec2 uv) {
    return min(max(min(min(logo_part(uv), logo_part(uv)), logo_part(uv)),
            sd_segment(uv, p7 * i_fac2, p2 * i_fac3 * i_fac2)),
        absmin(sd_segment(uv, i_p0, p2 * i_fac3 * i_fac),
            sd_segment(uv, p2 * i_fac3 * i_fac, p7 * i_fac)));
}
...
void main() {
    ...
    gl_FragColor = vec4(i_dis * i_Resolution.x + 2.5) / 5.;
}
