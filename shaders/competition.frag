...
vec3 ro=vec3(0,0,-6);
vec3 rd=normalize(vec3(((gl_FragCoord.xy.xy/vec2(i_X,i_Y))-1)*vec2(1,i_Y/i_X),1));
vec3 p=ro;
vec3 lp[10];
vec3 lc[3];
out vec3 color;
...
MA sdCappedCone( vec3 p, float h, float r1, float r2, float r3, vec3 col )
{
  ...
  float i_d = (cb.x<0.0 && ca.y<0.0) ? -1.0 : 1.0*sqrt( min(dot(ca,ca),dot(cb,cb)))-r3;
...
{
  vec3 q = abs(p) - b;
  float i_d= length(max(q,0.0)) + min(max(q.x,max(q.y,q.z)),0.0) - r;
...
{
      float i_e = pow(max(val - abs(f1 - f2), 0.0),2)*.25;
