...
opt_xz_lzma = [
    ...
    ["mf=bt4", "mf=hc4"],
    ["mode=normal"],
    ["nice=" + (str(nice)) for nice in [int for int in list(range(2, 274))]],
    ["depth=1000"],
]
...
opt_xz = list(
    map(
        lambda x: ["--stdout", "--keep"] + sum(x, []),
        itertools.product(
            [[], ["--x86"], ["--delta"], ["--x86", "--delta"], ["--delta", "--x86"]],
            map(
                lambda opts: ["--lzma2=" + ",".join(opts)],
...
opt_lzma = list(
    map(
        lambda opts: ["--stdout", "--keep", "--lzma1=" + ",".join(opts)],
        itertools.product(*opt_xz_lzma),
    )
)
