...
def run_compr(compr, mpool, inp):
    binf, opt, isgz = compr
    jobcnt = (len(opt))
    return list(tqdm(filter(lambda x: x[0] is not None,
                            mpool.imap(lambda o: (run_one(binf, o, inp), isgz), opt)), unit=' samples', maxinterval=1, mininterval=.1, desc='Autovndh', total=jobcnt))
...
def do_vndh(bin_nasm, vndh_dir, brute_result, vtag, vfork, vunibin, vcheat, vnoargv):
    ...
    if isgz == 2:
        cmdlin.append('-DUSE_ZSTD')
    elif isgz:
        cmdlin.append("-DUSE_GZIP")
    if vtag is not None:
        cmdlin.append('-DTAG='+vtag)
    if vfork:
        cmdlin.append('-DUSE_VFORK')
    if vunibin:
        cmdlin.append('-DNO_UBUNTU_COMPAT')
    if not vcheat:
        cmdlin.append('-DNO_CHEATING')
    if not vnoargv:
        cmdlin.append('-DWANT_ARGV')
...
opt_xz_lzma = [
    ...
    ['mf=bt4', 'mf=hc4'],
    ['mode=normal'],
    ['nice='+(str(nice)) for nice in [int for int in list(range(4, 273))]],
    ['depth=1000']
]
...
def main(opts):
    ...
    if verbose > 1:
        opt_zopfli[0].append('-v')

    if not is_exec(opts.nasm):
        eprint("nasm binary not found, doesn't exist or is not executable.")
        return

    if not is_exec(opts.gzip):
        opts.gzip = None
    if not is_exec(opts.xz):
        opts.xz = None
    if not is_exec(opts.lzma):
        opts.lzma = None
    if not is_exec(opts.zopfli):
        opts.zopfli = None
    ...
    if verbose > 0:
        eprint('final:',str(len(res)),'bytes')
...
""")
    ...
    p.add_argument('--vndh_tag', type=str,
